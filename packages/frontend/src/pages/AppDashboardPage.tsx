...
export function AppDashboardPage() {
  ...
  const activeJobs = tab === "available" ? filterAvailable(jobs) : tab === "client" ? clientJobs : workerJobs;
  const activeLoading = tab === "worker" ? workerJobsQuery.isLoading : allJobsQuery.isLoading;
  const activeError = tab === "worker" ? workerJobsQuery.error : allJobsQuery.error;
  const emptyCopy = tab === "available"
  ...
  async function claimFaucet() {
    if (!address || !canWrite || claimState.phase === "wallet" || claimState.phase === "confirming") return;
    ...
    try {
      ...
      setClaimState({ action: "claim", phase: "success" });
    } catch {
      setClaimState({ action: "claim", phase: "error" });
    }
  }

  const claimPending = claimState.phase === "wallet" || claimState.phase === "confirming";
  const claimLabel = claimState.phase === "wallet"
    ? "Confirm in wallet…"
    ...
    <div className="dashboard-page">
      ...
      <section className="balance-summary" aria-label="Wallet balances">
        <BalanceCard
          ...
          detail={address ? "Available for escrow funding" : "Connect a wallet to read your balance"}
          isLoading={usdcBalanceQuery.isLoading}
          error={usdcBalanceQuery.isError ? "USDC balance is unavailable." : undefined}
          action={canWrite ? { label: claimLabel, onClick: claimFaucet, disabled: claimPending } : undefined}
        />
        <BalanceCard
          label="GIWA ETH"
      ...
      <div className="claim-feedback" aria-live="polite">
        ...
        {claimState.phase === "error" ? "The faucet claim did not confirm. Check your wallet and try again." : null}
      </div>
      ...
      <section className="dashboard-jobs" aria-labelledby="job-list-heading">
        <div className="dashboard-jobs-heading">
          ...
          <div className="dashboard-tabs" role="tablist" aria-label="Job lists">
            {([
              ["available", "Available"],
              ["client", "My client jobs"],
              ["worker", "My worker jobs"],
        ...
        <div id="job-list" role="tabpanel" aria-labelledby={`${tab}-tab`}>
          <JobList jobs={activeJobs} isLoading={activeLoading} error={activeError} empty={emptyCopy} />
        </div>
      </section>
    </div>
  );
